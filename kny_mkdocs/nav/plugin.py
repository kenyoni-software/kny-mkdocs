    ...
    def on_page_markdown(self, markdown: str, /, *, page: Page, config: MkDocsConfig, files: Files) -> str | None:
        def render(items: list[str]) -> str:
            ...
            for item in items:
                ...
                rendered.append(f'<span class="kny-nav__item{active_class}">{html.escape(unescape(label))}</span>')
        ...
        def replace_brackets(match: re.Match[str]) -> str:
            # split on "/" that is not preceded by a backslash, keep the escapes for render()
            items = [part.strip() for part in re.split(r"(?<!\\)/", match.group(1))]
